document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('assignmentForm').addEventListener('submit', async (event) => {
        ...
        const assignmentData = {
            ...
            course: parseInt(formData.get('course'), 10),
            // course: formData.get('course'),
            // for_students: Array.from(formData.getAll('for_students')) 
        };

        try {
            const response = await fetch('https://hstu-six.vercel.app/api/assignments/', {
                method: 'POST',
                headers: {
...
async function fetchCourses() {
    try {
        const response = await fetch('https://hstu-six.vercel.app/api/courses/', {
            headers: {
                'Authorization': `Token ${localStorage.getItem('auth_token')}`
            }
        });
...
async function fetchStudents() {
    try {
        const response = await fetch('https://hstu-six.vercel.app/api/users/', {
            headers: {
                'Authorization': `Token ${localStorage.getItem('auth_token')}`
            }
        });
